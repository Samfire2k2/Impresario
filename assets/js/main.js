console.log('[main.js] Script carregado');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('[Drag & Drop] Initializing drag-drop inside DOMContentLoaded');
    
    const container = document.querySelector('.elements-container');
    if (container) {
        const cards = container.querySelectorAll('.element-card[draggable="true"]');
        console.log('[Drag & Drop] Found', cards.length, 'draggable cards');
        ...
        if (cards.length > 0) {
            ...
            cards.forEach(card => {
                card.addEventListener('dragstart', function(e) {
                    draggedElement = this;
                    draggedId = this.getAttribute('data-element-id');
                    console.log('[Drag & Drop] dragstart:', draggedId);
                ...
                card.addEventListener('dragend', function(e) {
                    ...
                    cards.forEach(c => c.classList.remove('drag-over', 'drag-over-bottom'));
                });
                ...
                card.addEventListener('dragover', function(e) {
                    ...
                    cards.forEach(c => c.classList.remove('drag-over', 'drag-over-bottom'));
                    this.classList.add(isTop ? 'drag-over' : 'drag-over-bottom');
                });
                ...
                card.addEventListener('drop', function(e) {
                    ...
                    console.log('[Drag & Drop] drop on:', this.getAttribute('data-element-id'));
                    
                    if (!draggedElement || this === draggedElement) {
                        console.log('[Drag & Drop] drop cancelled');
                        return;
                    }
                    
                    const draggedIdx = Array.from(cards).indexOf(draggedElement);
                    const targetIdx = Array.from(cards).indexOf(this);
                    
                    console.log('[Drag & Drop] dragged:', draggedIdx, 'target:', targetIdx);
                    ...
                    console.log('[Drag & Drop] moving', steps, 'steps', direction);
                    
                    // Fonction pour faire les déplacements
                    let step = 0;
                    
                    function doMove() {
                        if (step >= steps) {
                            console.log('[Drag & Drop] all moves done, reloading');
                            location.reload();
                            return;
                        }
                        
                        step++;
                        console.log('[Drag & Drop] move', step, '/', steps);
                        
                        fetch('api/positions.php', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                            body: 'action=move-element&element_id=' + draggedId + '&direction=' + direction
                        })
                        .then(r => r.json())
                        .then(data => {
                            ...
                            if (data.success) {
                                ...
                                alert('Erreur: ' + (data.error || 'Impossible de déplacer'));
                            }
                        })
                        .catch(err => {
            ...
            console.log('[Drag & Drop] ✓ Drag-drop initialized successfully');
        }
    } else {
        console.log('[Drag & Drop] No .elements-container found');
    }
});
