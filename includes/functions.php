...
// Fonction pour générer une couleur de post-it à partir de l'ID de l'élément
function getPostItColor($elementId) {
    $colors = [
        '#fffbf0', // Cream original
        '#fff8e1', // Yellow
        '#f0f8ff', // Alice blue
        '#fce4ec', // Light pink
        '#f3e5f5', // Light purple
    ...
    $index = abs($elementId) % count($colors);
    return $colors[$index];
}
...
function verifyLogin($pdo, $username, $password) {
    ...
    if ($user) {
        // Support pour mots de passe hashés ET en clair (pour développement)
...
function createElement($pdo, $intrigueId, $type, $title, $description = '') {
    ...
    if ($intrigueId !== null) {
        ...
        $stmt = $pdo->prepare('
            SELECT COALESCE(MAX(position), 0) + 1 as new_position FROM element WHERE intrigue_id IS NULL
        ');
        $stmt->execute([]);
    }
    $result = $stmt->fetch();
...
function getProjectWritingStatus($pdo, $projectId) {
    ...
    // Add writing status counts (placeholder - column doesn't exist yet)
    $result['draft_count'] = 0;
    $result['first_read_count'] = 0;
